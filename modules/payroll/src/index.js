...
const overtime_js_1 = require("./detectors/overtime.js");
const contract_expiry_js_1 = require("./detectors/contract-expiry.js");
const salary_anomaly_js_1 = require("./detectors/salary-anomaly.js");
// Module state
...
exports.PayrollModule = {
     ...
     */
    async registerModule(context) {
        moduleContext = context;
        const config = context.config;
        context.logger.info('Initializing Payroll module');
        // Initialize risk detectors
        detectors = [
            new overtime_js_1.OvertimeDetector(context, config.overtime_threshold_hours),
            new contract_expiry_js_1.ContractExpiryDetector(context, config.contract_expiry_warning_days),
            new salary_anomaly_js_1.SalaryAnomalyDetector(context, config.salary_anomaly_threshold_percent),
        ];
        ...
        context.eventBus.subscribe('employee.created', async (payload) => {
            context.logger.info('Employee created event received', { payload });
            // Could trigger re-detection here
        });
        context.eventBus.subscribe('contract.updated', async (payload) => {
     ...
     */
    async getRiskFindings(tenantId, timeframe) {
        if (!moduleContext) {
    ...
    async getModuleHealth() {
        ...
        const detectorHealthResults = await Promise.all(detectors.map(async (d) => ({
            id: d.id,
    ...
    return [
        // GET /api/modules/payroll/employees
        {
            ...
            handler: async (req, res) => {
                ...
                const employees = await context.db.query(`SELECT * FROM employees
           WHERE tenant_id = ?
        ...
        // GET /api/modules/payroll/overtime
        {
            ...
            handler: async (req, res) => {
                ...
                const overtime = await context.db.query(`SELECT
            e.id as employee_id,
           ...
           ORDER BY overtime_hours DESC`, [tenantId, from_date, to_date]);
...
// Export module
exports.default = exports.PayrollModule;
