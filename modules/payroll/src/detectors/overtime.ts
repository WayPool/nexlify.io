...
export class OvertimeDetector implements RiskDetector {
  ...
  constructor(context: ModuleContext, thresholdHours: number = 40) {
  ...
  async detect(tenantId: string, timeframe: TimeFrame): Promise<RiskFinding[]> {
    ...
    const overtimeData = await this.context.db.query<{
      employee_id: string;
      ...
      `SELECT
       ...
       HAVING total_hours > ?`,
      [tenantId, timeframe.from, timeframe.to, this.thresholdHours]
    );
   ...
   */
  private calculatePotentialFine(overtimeHours: number): number {
    if (overtimeHours > 40) {
