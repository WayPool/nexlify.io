...
export class ContractExpiryDetector implements RiskDetector {
  ...
  constructor(context: ModuleContext, warningDays: number = 30) {
    this.context = context;
    this.warningDays = warningDays;
  }

  async detect(tenantId: string, _timeframe: TimeFrame): Promise<RiskFinding[]> {
    this.context.logger.debug('Running contract expiry detection', { tenantId });

    // Query contracts expiring soon
    const expiringContracts = await this.context.db.query<{
      contract_id: string;
      employee_id: string;
