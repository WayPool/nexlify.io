...
import { createRiskFinding } from '@platform/sdk';

export class SalaryAnomalyDetector implements RiskDetector {
  ...
  constructor(context: ModuleContext, thresholdPercent: number = 15) {
    this.context = context;
    this.thresholdPercent = thresholdPercent;
  }

  async detect(tenantId: string, timeframe: TimeFrame): Promise<RiskFinding[]> {
   ...
   */
  private async detectBelowAverage(tenantId: string): Promise<RiskFinding[]> {
    const results = await this.context.db.query<{
      employee_id: string;
      ...
      `SELECT
       ...
       WHERE e.tenant_id = ?
       AND ((dept.avg_salary - c.monthly_salary) / dept.avg_salary * 100) > ?`,
      [tenantId, tenantId, this.thresholdPercent]
    );
   ...
   */
  private async detectLargeChanges(
    tenantId: string,
    timeframe: TimeFrame
  ): Promise<RiskFinding[]> {
    const results = await this.context.db.query<{
      ...
      `SELECT
       ...
       AND ABS((sh.new_salary - sh.old_salary) / sh.old_salary * 100) > ?`,
      [tenantId, timeframe.from, timeframe.to, this.thresholdPercent * 2]
    );
   ...
   */
  private async detectPayGap(tenantId: string): Promise<RiskFinding[]> {
    const results = await this.context.db.query<{
      department: string;
      role: string;
      male_avg: number;
      ...
      `SELECT
       ...
       AND ABS(gap_percent) > ?`,
      [tenantId, this.thresholdPercent]
    );
  ...
  private calculatePayEquityRisk(salary: number, avgSalary: number): number {
    const annualDifference = (avgSalary - salary) * 12;
    // Potential back-pay plus legal costs
    return annualDifference * 2 + 5000;
  }

  private calculatePayGapLiability(gapPercent: number): number {
