...
export class TransactionSyncService {
  private nordigen: NordigenClient;
   ...
   */
  private async refreshAccounts(connection: BankConnection): Promise<void> {
  ...
  async syncAccount(account: BankAccount): Promise<SyncResult> {
    ...
    try {
      ...
      const { transactions } = await this.nordigen.getAccountTransactions(
        account.externalId,
        dateFrom,
        dateTo
      );
      ...
      for (const nordigenTx of transactions.booked) {
        try {
          ...
          result.errors.push(
            `Failed to process transaction ${nordigenTx.transactionId}: ${error instanceof Error ? error.message : 'Unknown error'}`
          );
        }
      }
      ...
      for (const nordigenTx of transactions.pending) {
        try {
          ...
          result.errors.push(
            `Failed to process pending transaction: ${error instanceof Error ? error.message : 'Unknown error'}`
          );
        }
      }
      ...
      if (currentBalance) {
        await this.store.updateAccountBalance(
          account.id,
          parseFloat(currentBalance.balanceAmount.amount),
          currentBalance.referenceDate
        );
      }
    } catch (error) {
  ...
  private mapTransaction(
    nordigenTx: NordigenTransaction,
    accountId: string,
   ...
   */
  private extractMerchantName(tx: NordigenTransaction): string | null {
   ...
   */
  private categorizeTransaction(tx: NordigenTransaction): TransactionCategory | null {
    const description = (
   ...
   */
  private mapAccountType(cashAccountType?: string): 'checking' | 'savings' | 'credit' | 'loan' | 'other' {
    if (!cashAccountType) return 'checking';

    const typeMap: Record<string, 'checking' | 'savings' | 'credit' | 'loan' | 'other'> = {
      CACC: 'checking',
      SVGS: 'savings',
      CARD: 'credit',
      LOAN: 'loan',
