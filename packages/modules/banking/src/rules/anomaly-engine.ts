...
export class AnomalyEngine {
  ...
  async analyzeTransaction(
    ...
    tenantId: string
  ): Promise<{ flags: TransactionFlag[]; score: number }> {
  ...
  async analyzeTransactions(
    ...
    tenantId: string
  ): Promise<Map<string, { flags: TransactionFlag[]; score: number }>> {
    const results = new Map<string, { flags: TransactionFlag[]; score: number }>();
   ...
   */
  private async detectUnusualAmount(tx: Transaction): Promise<TransactionFlag | null> {
    const avgAmount = await this.history.getAverageAmount(tx.accountId, tx.direction);

    if (avgAmount === 0) return null; // No history
   ...
   */
  private detectUnusualTime(tx: Transaction): TransactionFlag | null {
   ...
   */
  private async detectUnknownCounterparty(tx: Transaction): Promise<TransactionFlag | null> {
    if (!tx.counterpartyName && !tx.counterpartyIban) return null;
   ...
   */
  private detectHighRiskCountry(tx: Transaction): TransactionFlag | null {
   ...
   */
  private detectRoundAmount(tx: Transaction): TransactionFlag | null {
    // Only flag larger amounts
    if (tx.amount < 1000) return null;
   ...
   */
  private detectStructuring(tx: Transaction): TransactionFlag | null {
   ...
   */
  private async detectDormantActivation(tx: Transaction): Promise<TransactionFlag | null> {
   ...
   */
  private async detectDuplicate(tx: Transaction): Promise<TransactionFlag | null> {
    ...
    const duplicates = recentTxs.filter((other) => {
      ...
      if (other.direction !== tx.direction) return false;
      if (other.counterpartyIban !== tx.counterpartyIban) return false;

      const daysDiff = Math.abs(
        differenceInDays(parseISO(tx.bookingDate), parseISO(other.bookingDate))
      );
      return daysDiff <= 1;
    });
   ...
   */
  private async runCustomRules(tx: Transaction, tenantId: string): Promise<TransactionFlag[]> {
    const flags: TransactionFlag[] = [];

    // Filter rules for this tenant (or global rules)
    const applicableRules = this.rules.filter(
      (rule) => rule.tenantId === null || rule.tenantId === tenantId
    );
   ...
   */
  private evaluateRule(rule: AnomalyRule, tx: Transaction): boolean {
   ...
   */
  private evaluateCondition(condition: RuleCondition, tx: Transaction): boolean {
   ...
   */
  private getFieldValue(tx: Transaction, field: string): unknown {
   ...
   */
  private calculateAnomalyScore(flags: TransactionFlag[]): number {
 ...
 */
export function getDefaultRules(): Omit<AnomalyRule, 'id' | 'createdAt' | 'updatedAt'>[] {
  return [
    {
